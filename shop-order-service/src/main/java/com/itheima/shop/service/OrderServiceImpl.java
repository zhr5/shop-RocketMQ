...
public class OrderServiceImpl implements IOrderService {
    ...
    private void reduceMoneyPaid(TradeOrder order) {
        if (order.getMoneyPaid() != null && order.getMoneyPaid().compareTo(BigDecimal.ZERO) == 1) {
            ...
            userMoneyLog.setMoneyLogType(ShopCode.SHOP_USER_MONEY_PAID.getCode()); // 类型
    ...
    private void updateCouponStatus(TradeOrder order) {
        if (order.getCouponId() != null) {  // 是否使用优惠券
    ...
    private void reduceGoodsNum(TradeOrder order) {
        ...
        goodsNumberLog.setGoodsNumber(order.getGoodsNumber()); // 商品数量
